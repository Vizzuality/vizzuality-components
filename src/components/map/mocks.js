export const layers = [
  {
    ...
    description:
      'First-level subnational political boundaries are the largest administrative subdivisions in a country.',
    application: ['rw'],
    iso: ['global'],
    provider: 'cartodb',
        ...
        layers: [
          {
            options: {
              ...
              cartocss:
                '#gadm28_adm1{  polygon-fill: #3bb2d0;  polygon-opacity: 0;  line-color: #5CA2D1;  line-width: 0.5;  line-opacity: 1;}',
  ...
  },
  {
    ...
    description:
      'Modeled average abundance of originally present species, relative to their abundance in an intact ecosystem.',
    application: ['rw'],
    iso: [],
    provider: 'gee',
    ...
    layerConfig: {
      ...
      body: {
        styleType: 'sld',
        sldValue:
          '<RasterSymbolizer><ColorMap type="ramp" extended="false" ><ColorMapEntry color="#ffffcc" quantity="0.6"  opacity="1" /><ColorMapEntry color="#c2e699" quantity="0.7" /><ColorMapEntry color="#78c679" quantity="0.8" /><ColorMapEntry color="#31a354" quantity="0.9"  /><ColorMapEntry color="#006837" quantity="1.0"  /></ColorMap></RasterSymbolizer>',
      },
    },
    legendConfig: {
      type: 'choropleth',
  ...
  },
  {
    ...
    interactionConfig: {
      ...
      config: {
        url:
          'https://staging-api.globalforestwatch.org/v1/query/b5458f7f-b5e6-4b29-8524-5040e4bf5a27?sql=select first(b1) as x from \'users/resourcewatch_wri/foo_024_vegetation_health_index\' where system:time_start >= 1532822500000 and ST_INTERSECTS(ST_SetSRID(ST_GeomFromGeoJSON(\'{"type":"Point","coordinates":[{{lng}},{{lat}}]}\'),4326), the_geom)',
      ...
      pulseConfig: {
        url:
          'https://api.resourcewatch.org/v1/query/4828c405-06a2-4460-a78c-90969bce582b?sql=select first(b1) as x from \'users/resourcewatch_wri/foo_024_vegetation_health_index\' where system:time_start >= 1533448800000 and st_intersects(the_geom,st_buffer(ST_SetSRID(st_geomfromgeojson(\'{"type":"Point","coordinates":{{point}}}\'),4326),1))',
      },
      output: [
        {
          column: 'x',
